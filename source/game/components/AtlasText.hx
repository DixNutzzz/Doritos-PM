...
/** PLACEHOLDER **/
class AtlasText extends FlxSpriteContainer
{
...
class AlphaCharacter extends FlxSprite
{
	...
	{
		var path:String = Paths.file('data/$request.json', TEXT);
		if(!FlxG.assets.exists(path, TEXT))
			path = Paths.file('data/alphabet.json', TEXT);

		allLetters = new Map<String, Null<Letter>>();
		try
		{
			var data:Dynamic = Json.parse(FlxG.assets.getText(path));
